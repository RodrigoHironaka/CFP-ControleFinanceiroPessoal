...
alter table Caixas  drop foreign key FK_864EAC36
;

alter table Caixas  drop foreign key FK_FE47561F
;
...
create table FluxoCaixas (Id BIGINT not null, Nome VARCHAR(150), DataGeracao DATETIME, DataAlteracao DATETIME, Caixa BIGINT, UsuarioLogado BIGINT, primary key (Id)) ENGINE=InnoDB;
...
create table Caixas (ID BIGINT not null, Codigo BIGINT, DataAbertura DATETIME, DataFechamento DATETIME, TotalEntrada DECIMAL(19,5), TotalSaida DECIMAL(19,5), BalançoFinal DECIMAL(19,5), Situacao INTEGER, Pessoa BIGINT, Conta BIGINT, UsuarioLogado BIGINT, primary key (ID)) ENGINE=InnoDB;
...
alter table Caixas add index (Conta), add constraint FK_864EAC36 foreign key (Conta) references Contas (Id);
alter table Caixas add index (UsuarioLogado), add constraint FK_FE47561F foreign key (UsuarioLogado) references Usuarios (Id);
