...
alter table Configuracoes  drop foreign key FK_E7DFA2C3
;
...
alter table Contas  drop foreign key FK_70B890B
;
...
create table Configuracoes (Id BIGINT not null, DataGeracao DATETIME, DataAlteracao DATETIME, UsuarioCriacao BIGINT, CaminhoArquivos VARCHAR(250), CaminhoBackup VARCHAR(250), FormaPagamentoPadraoConta BIGINT, TransacaoBancariaPadrao BIGINT, DiasAlertaVencimento INTEGER, GrupoGastoFaturaPadrao BIGINT, HorasTrabalhadasPorPeriodo BIGINT, HorasTrabalhadasPorDia BIGINT, primary key (Id)) ENGINE=InnoDB;
...
create table Bancos (Id BIGINT not null, Nome VARCHAR(70), DataGeracao DATETIME, DataAlteracao DATETIME, TipoContaBanco INTEGER, Situacao INTEGER, UsarValorParaCalculos INTEGER, Pessoa BIGINT, UsuarioCriacao BIGINT, UsuarioAlteracao BIGINT, primary key (Id)) ENGINE=InnoDB;
...
create table Cofres (Id BIGINT not null, Codigo BIGINT, Caixa BIGINT, Banco BIGINT, Valor DECIMAL(10, 2), DataRegistro DATETIME, TransacoesBancarias BIGINT, Situacao INTEGER, Nome VARCHAR(200), DataGeracao DATETIME, DataAlteracao DATETIME, UsuarioCriacao BIGINT, UsuarioAlteracao BIGINT, primary key (Id)) ENGINE=InnoDB;
create table Contas (Id BIGINT not null, Codigo BIGINT, TipoConta INTEGER, TipoPeriodo INTEGER, Situacao INTEGER, DataEmissao DATETIME, ValorTotal DECIMAL(10, 2), QtdParcelas BIGINT, NumeroDocumento BIGINT, SubGrupoGasto BIGINT, FormaCompra BIGINT, FaturaCartaoCredito BIGINT, Pessoa BIGINT, Observacao TEXT, Nome VARCHAR(70), DataGeracao DATETIME, DataAlteracao DATETIME, UsuarioCriacao BIGINT, UsuarioAlteracao BIGINT, primary key (Id)) ENGINE=InnoDB;
...
alter table Configuracoes add index (GrupoGastoFaturaPadrao), add constraint FK_E7DFA2C3 foreign key (GrupoGastoFaturaPadrao) references SubGrupoGastos (Id);
...
alter table Contas add index (FaturaCartaoCredito), add constraint FK_70B890B foreign key (FaturaCartaoCredito) references CartoesCredito (Id);
